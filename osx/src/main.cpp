...
void run_camera(int width, int height, int fps, Uint32 duration)
{
	...
	while (ctx.running) {
        ...
        for(int i = 0; i < size; ++i){
            if(i%3==0){
                int average = (destimg[i] + destimg[i+1] + destimg[i+2])/3;
                destimg[i] = average;
                destimg[i+1] = average;
                destimg[i+2] = average;
            }
        ...
        // for(int j = 0; j < ctx.eye->getHeight(); ++j){
        //     for(int i = 0; i < ctx.eye->getWidth()/2; ++i){
        //         pixel = &(destimg[j*ctx.eye->getWidth()*3 + i*3]);

        //         flip_pixel = &(destimg[j*ctx.eye->getWidth()*3 + (ctx.eye->getWidth()-i-1)*3]);
        //         for(int k=0; k<3; ++k){
        //             tmp = *(pixel + k);
